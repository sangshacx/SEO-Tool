import {
  BacklinkDetailsProviderError,
  fetchBacklinkDetails,
} from "../../../../src/v2/providers/dataforseo-backlink-details.js";
import {
  backlinkDetailsCacheKey,
...
const ENDPOINT_NAME = "backlinks/backlinks/live";
const CACHE_TTL_SECONDS = 7 * 24 * 60 * 60;
const MAX_BODY_BYTES = 64 * 1024;
const ALLOWED_LIMITS = new Set([25, 50, 100]);
const ALLOWED_SORTS = new Set(["rank", "domain_rank", "page_rank", "first_seen", "last_seen", "spam_score"]);
const ALLOWED_STATUSES = new Set(["live", "lost"]);
const ALLOWED_FOLLOW = new Set(["all", "dofollow", "nofollow"]);
...
async function logUsage(env, values) {
  try {
    await recordApiUsage({
      ...
      operation: "backlink_details",
    });
  } catch (error) {
    console.error(JSON.stringify({
      message: "backlink details usage logging failed",
...
export async function onRequestPost({ request, env }) {
  ...
  const follow = typeof body?.follow === "string" ? body.follow : "all";
  ...
  if (!ALLOWED_SORTS.has(sort)) {
    return json({ ok: false, error: { code: "INVALID_SORT", field: "sort", message: "Choose a supported backlink sort." } }, 400);
  }
  if (!ALLOWED_STATUSES.has(status)) {
    return json({ ok: false, error: { code: "INVALID_STATUS", field: "status", message: "Choose live or lost backlinks." } }, 400);
  }
  if (!ALLOWED_FOLLOW.has(follow)) {
    return json({ ok: false, error: { code: "INVALID_FOLLOW", field: "follow", message: "Choose all, dofollow, or nofollow backlinks." } }, 400);
  }

  const key = backlinkDetailsCacheKey({ domain, limit, offset, sort, status, follow });
  ...
  if (body?.allow_live_request !== true) {
    return json({
      ok: false,
      error: { code: "LIVE_REQUEST_CONFIRMATION_REQUIRED", message: "No cached backlink details page exists. Allow a live request to continue." },
      meta: { request_id: requestId, cached: false, actual_cost_usd: 0, cache_ttl_days: 7 },
    }, 409);
  }

  try {
    const provider = await fetchBacklinkDetails({
    ...
    const providerError = error instanceof BacklinkDetailsProviderError ? error : null;
    ...
    console.error(JSON.stringify({
      message: "backlink details request failed",
      request_id: requestId,
      code: providerError?.code ?? "BACKLINK_DETAILS_FAILED",
      provider_status: providerError?.providerStatus ?? null,
    }));
    return json({
      ok: false,
      error: { code: providerError?.code ?? "BACKLINK_DETAILS_FAILED", message: "Backlink details could not be loaded." },
...
export function onRequestGet() {
  return json({ ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "Use POST for backlink details." } }, 405);
}
