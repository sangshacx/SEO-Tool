...
import { buildOrganicPositionChanges } from "../../../../src/v2/organic/organic-position-changes.js";
...
async function logUsage(env, values) {
  try {
    await recordApiUsage({
      ...
      operation: "organic_position_changes",
    });
  } catch (error) {
    console.error(JSON.stringify({
      message: "organic position changes usage logging failed",
...
async function readHierarchicalCache(cache, input) {
  for (const candidate of organicKeywordsCacheCandidates(input)) {
    ...
    return {
      data: buildOrganicPositionChanges(projectOrganicKeywordsDepth(data, input.depth)),
...
export async function onRequestPost({ request, env }) {
  ...
  if (!normalizedTarget) {
    return json({ ok: false, error: { code: "VALIDATION_ERROR", field: "target", message: "Enter a valid domain, subdomain, or absolute page URL." } }, 400);
  }
  ...
  if (body?.allow_live_request !== true) {
    return json({
      ok: false,
      error: { code: "LIVE_REQUEST_CONFIRMATION_REQUIRED", message: "No compatible 7-day Position Changes cache exists. Allow a live request to continue." },
  ...
  try {
    ...
    const data = buildOrganicPositionChanges(provider.data);
    await logUsage(env, {
    ...
    return json({
      ok: false,
      error: { code: providerError?.code ?? "ORGANIC_POSITION_CHANGES_FAILED", message: "Position Changes could not be loaded." },
...
export function onRequestGet() {
  return json({ ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "Use POST for Position Changes." } }, 405);
}
