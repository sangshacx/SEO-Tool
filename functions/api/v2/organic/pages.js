import {
  RelevantPagesProviderError,
  RELEVANT_PAGE_DEPTHS,
  fetchRelevantPages,
...
const ENDPOINT_NAME = "dataforseo_labs/google/relevant_pages/live";
const JSON_HEADERS = {
...
async function logUsage(env, values) {
  try {
    await recordApiUsage({
      ...
      operation: "organic_pages",
    });
  } catch (error) {
    console.error(JSON.stringify({
      message: "organic pages usage logging failed",
...
async function readHierarchicalCache(cache, input) {
  for (const candidate of organicPagesCacheCandidates(input)) {
    ...
    return {
      data: projectOrganicPagesDepth(data, input.depth),
...
export async function onRequestPost({ request, env }) {
  ...
  if (!Number.isInteger(depth) || !RELEVANT_PAGE_DEPTHS.includes(depth)) {
    return json({ ok: false, error: { code: "INVALID_DEPTH", field: "depth", message: "Choose Quick 100, Standard 500, or Deep 1000." } }, 400);
  }
  ...
  const cacheInput = { target: domain, locationCode, languageCode, depth };
  ...
  if (cached) {
    ...
    return json({
      ...
      data: cached.data,
      meta: {
        request_id: requestId, cached: true, cached_at: cached.cachedAt,
        cached_from_depth: cached.cachedFromDepth, requested_depth: depth,
        actual_cost_usd: 0, cache_ttl_days: 7, provider_requests: 0,
  ...
  if (body?.allow_live_request !== true) {
    return json({
      ok: false,
      error: { code: "LIVE_REQUEST_CONFIRMATION_REQUIRED", message: "No compatible 7-day Top Pages cache exists. Allow a live request to continue." },
      meta: { request_id: requestId, cached: false, requested_depth: depth, actual_cost_usd: 0, cache_ttl_days: 7, provider_requests: 0 },
    }, 409);
  }

  try {
    const provider = await fetchRelevantPages({
    ...
    const cacheKey = buildOrganicPagesCacheKey(cacheInput);
    ...
    await logUsage(env, {
      ...
      actualCostUsd: provider.actualCostUsd, cacheHit: false, status: "success",
      httpStatus: 200, durationMs: Date.now() - startedAt,
    });
    return json({
      ok: true,
      data: provider.data,
      meta: {
        ...
        cached_from_depth: depth, requested_depth: depth,
        actual_cost_usd: provider.actualCostUsd, cache_ttl_days: 7,
        provider_requests: 1, duration_ms: Date.now() - startedAt,
      },
    });
  } catch (error) {
    const providerError = error instanceof RelevantPagesProviderError ? error : null;
    const httpStatus = providerError?.httpStatus ?? 502;
    await logUsage(env, {
      requestId,
      taskCount: providerError?.code === "PROVIDER_CREDENTIALS_MISSING" ? 0 : 1,
      resultCount: 0, actualCostUsd: providerError?.actualCostUsd ?? null,
      cacheHit: false, status: "error", httpStatus,
      durationMs: Date.now() - startedAt,
    });
    return json({
      ok: false,
      error: { code: providerError?.code ?? "ORGANIC_PAGES_FAILED", message: "Top Pages could not be loaded." },
...
export function onRequestGet() {
  return json({ ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "Use POST for Top Pages." } }, 405);
}
