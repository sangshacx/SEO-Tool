...
const ENDPOINT_NAME = "ai_optimization/llm_mentions/top_mentioned_pages/live";
...
async function logUsage(env, values) {
  try {
    await recordApiUsage({
      ...
      operation: "ai_visibility_pages",
    });
  } catch (error) {
    console.error(JSON.stringify({
      message: "AI visibility pages usage logging failed",
...
function validate(body) {
  ...
  if (!AI_VISIBILITY_TOP_PAGE_LIMITS.includes(limit)) {
    const error = new Error("Choose 10, 25, 50, or 100 mentioned pages.");
...
export async function onRequestPost({ request, env }) {
  ...
  try { scope = validate(body); }
    ...
    return json({
      ok: false,
      error: { code: known ? error.code : error?.code ?? "VALIDATION_ERROR", message: error?.message ?? "Invalid top mentioned pages request." },
    ...
    return json({
      ...
      error: {
        ...
        message: forceRefresh
          ? "A paid top mentioned pages refresh requires explicit confirmation."
          : "No compatible top mentioned pages cache exists. Allow a paid live request to continue.",
      },
      meta: {
        request_id: requestId, cached: false, force_refresh: forceRefresh,
        actual_cost_usd: 0, provider_requests: 0,
        billing_note: "Top Mentioned Pages is a paid DataForSEO request and row count affects provider cost.",
      },
    }, 409);
  }

  try {
    const provider = await fetchAiVisibilityTopMentionedPages({
    ...
    return json({
      ok: false,
      error: {
        code: providerError?.code ?? "AI_VISIBILITY_PAGES_FAILED",
        message: providerError?.message ?? "Top mentioned pages could not be loaded.",
      },
...
export function onRequestGet() {
  return json({ ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "Use POST for top mentioned pages." } }, 405);
}
