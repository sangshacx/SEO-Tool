...
export { buildCompetitorSnapshotCacheKey };
...
export async function onRequestPost({ request, env }) {
  ...
  const cachedData = cached && typeof cached === "object" && Object.hasOwn(cached, "data") ? cached.data : cached;
  ...
  try {
    ...
    const response = await fetch(ENDPOINT, { method: "POST", headers: { Authorization: "Basic " + btoa(env.DATAFORSEO_LOGIN + ":" + env.DATAFORSEO_PASSWORD), "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify([{ target: domain, location_code: locationCode, language_code: languageCode, item_types: ["organic"], ignore_synonyms: true, include_clickstream_data: false, limit: 50, order_by: ["ranked_serp_element.serp_item.rank_group,asc", "keyword_data.keyword_info.search_volume,desc"], tag: "seo-pro-v2-competitor-snapshot" }]) });
    ...
    const data = enrichSnapshot(normalizeResult(task.result?.[0], domain)), cost = typeof payload.cost === "number" ? payload.cost : (typeof task.cost === "number" ? task.cost : null);
    await env.CACHE.put(cacheKey, JSON.stringify({ data, cached_at: new Date().toISOString() }), { expirationTtl: CACHE_TTL_SECONDS });
    await usage(env, { requestId, taskCount: Number.isInteger(payload.tasks_count) ? payload.tasks_count : null, resultCount: data.top_keywords.length, actualCostUsd: cost, cacheHit: false, status: "success", httpStatus: 200, durationMs: Date.now() - startedAt });
    ...
    await usage(env, { requestId, taskCount: null, resultCount: 0, actualCostUsd: null, cacheHit: false, status: "error", httpStatus: status, durationMs: Date.now() - startedAt });
