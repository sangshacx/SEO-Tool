...
export async function onRequestPost({ request, env }) {
  ...
  const domain = normalizeDomain(body?.domain), locationCode = Number.isInteger(body?.location_code) ? body.location_code : 2840, languageCode = typeof body?.language_code === "string" ? body.language_code : "en";
  ...
  const cacheKey = ["v2", "competitor-snapshot", domain, locationCode, languageCode].join(":"), cached = await env.CACHE.get(cacheKey, "json");
